...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) 
{
    ...
    elseif(!isset($_POST['p_admin']) || !preg_match("/^[0-9]+$/", $_POST["p_admin"]) || $_POST["p_admin"] < 0 || $_POST["p_admin"] > 1)
    {
        echo json_encode(array("resp"=>"Helytelen érték: p_admin!"));
    }
    else
    {
        $con = connect();
        ...
        $res = mysqli_query($con, "SELECT modify_admin_law('" . mysqli_real_escape_string($con, $_POST['p_admin']) . "', '" . mysqli_real_escape_string($con, $_POST['p_userid']) . "', '" . mysqli_real_escape_string($con, $_SESSION['user_id']) . "') AS response");
        mysqli_close($con);
        if(!$res)
        {
            die(err_db());
