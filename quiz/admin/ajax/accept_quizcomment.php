...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) 
{
    ...
    else
    {
        ...
        $res = mysqli_query($con, "SELECT accept_quiz_comment('" . mysqli_real_escape_string($con, $_POST['p_quizid']) . "', '" . mysqli_real_escape_string($con, $_POST['p_userid']) . "', '" . mysqli_real_escape_string($con, $_POST['p_commentdate']) . "', '" . mysqli_real_escape_string($con, $_SESSION['user_id']) . "') AS p_response");
        mysqli_close($con);
        if(!$res)
        {
            die(err_db());
