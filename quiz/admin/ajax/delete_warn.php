...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) 
{
    ...
    elseif(!isset($_POST['p_delreason']) || strlen($_POST["p_delreason"]) < 5 || strlen($_POST["p_delreason"]) > 100)
    {
        echo json_encode(array("resp"=>"Helytelen érték: p_delreason!"));
    }
    else
    {
        $con = connect();
        ...
        $res = mysqli_query($con, "SELECT delete_warn('" . mysqli_real_escape_string($con, $_POST['p_userid']) . "', '" . mysqli_real_escape_string($con, $_SESSION['user_id']) . "', '" . mysqli_real_escape_string($con, $_POST['p_delreason']) . "') AS response");
        mysqli_close($con);
        if(!$res)
        {
            die(err_db());
