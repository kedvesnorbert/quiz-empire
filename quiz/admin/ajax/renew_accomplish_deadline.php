...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) 
{
    ...
    elseif(!isset($_POST['p_days']) || !preg_match("/^[0-9]+$/", $_POST["p_days"]) || $_POST["p_days"] < 1 )
    {
        echo json_encode(array("resp"=>"Helytelen érték: p_days!"));
    }
    else
    {
        $con = connect();
        ...
        mysqli_query($con, "CALL renew_accomplish_deadline('" . mysqli_real_escape_string($con, $_POST['p_quizid']) . "', '" . mysqli_real_escape_string($con, $_SESSION['adminuser']) . "', '" . mysqli_real_escape_string($con, $_POST['p_days']) . "', @response)");
