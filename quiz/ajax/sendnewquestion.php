...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) 
{
	...
	elseif (!isset($_POST["tema"]) || empty($_POST["tema"]) || $_POST["tema"] < 1)
	{
		echo json_encode(array("resp"=>"HIBA! Nincs kiválasztva a témakör!"));
	}
	elseif (!preg_match("/^[0-9]+$/", $_POST['tema']))
	{
	...
	elseif (!isset($_POST["diff"]) || empty($_POST["diff"]) || !preg_match("/^[0-9]+$/", $_POST['diff']) || $_POST["diff"] < 1 || $_POST["diff"] > 2)
	{
		echo json_encode(array("resp"=>"HIBA! Nincs kiválasztva a kérdés nehézsége!"));
	}
	else
	{
		$con = connect();
		...
		$r = mysqli_query($con, "SELECT send_question('" . mysqli_real_escape_string($con, $_POST['tema']) . "', '" . mysqli_real_escape_string($con, $_POST['question']) . "', '" . mysqli_real_escape_string($con, $_POST['ans1']) . "', '" . mysqli_real_escape_string($con, $_POST['ans2']) . "', '" . mysqli_real_escape_string($con, $_POST['ans3']) . "', '" . mysqli_real_escape_string($con, $_POST['ans4']) . "', '" . mysqli_real_escape_string($con, $_SESSION['user']) . "', '" . mysqli_real_escape_string($con, $_POST['megj']) . "', '" . mysqli_real_escape_string($con, $_POST['diff']) . "') AS p_response");
		$ro = mysqli_fetch_assoc($r);
		mysqli_close($con);
		echo json_encode(array("resp"=>$ro["p_response"]));
	}
}
else
{
