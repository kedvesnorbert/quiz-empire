...
function show_mynews()
{
	$deviceType = checkDevice();
	$res = db_getMyNewsData();
	if(!$res)
	{
		die(err_db());
	}
	while($row = mysqli_fetch_assoc($res))
	{
		...
		<div id='one_news_div' class="<?php echo "one_new_divclass" . $row['hir_id'] ?>">
			...
			<tr>
				...
				<td>
				<?php
				if($row['username'] == $_SESSION['user'] || $_SESSION["admin_user"] == 1)
				{
					?><button id='delmy_news' class="btn btn-danger" onclick='delete_this_news("<?php echo $row["hir_id"]; ?>")'>Törlés</button>
